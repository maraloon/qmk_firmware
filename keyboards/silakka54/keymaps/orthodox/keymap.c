 ...
 C(Bs),    N,     R,     T,    St,     G,            Y,     H,     A,     E,     I,   oS,
  ...
  [RUS] = LAYOUT(
    ...
    rT,    Z,     X,    Ct,     D,     V,            K,     H,     O,    rU,    rJ,  ExlmNS,
             DotNS, Space, OSM(MOD_LSFT),            _, _, _
  ),
...
//       --   <-   ==   ->   ++         -[]   { }       {}           <>
  ...
  [TMUX] = LAYOUT(
    ...
    _, LALT(Q), LALT(X), LALT(M), LALT(Ct), LALT(V),       LALT(K), oCA, LALT(P), _, _, _,
                 LALT(Z), QK_LLCK, LALT(Space),            STRES, _, _
  ),
